...
static BList *cmdLineOpen = NULL;
//static const char *projectDirOpen = NULL;
...
int main(int argc, char **argv)
{
	...
	if (argc >= 2)
	{
		for(int i=1;i<argc;i++)
		{
			...
			else if (!strcmp(parm, "-big"))
			{
				editor.settings.testmode = 2;
			}
			else if (parm[0] != '-')
			{
				/*if (dir_exists(parm) && ProjectManager.IsExistingProject(parm) && !projectDirOpen)
				{
					stat("Interpreting parameter '%s' as an implicit request to open that project", parm);
					projectDirOpen = parm;
				}
				else*/
				{
					if (!cmdLineOpen) cmdLineOpen = new BList;
					cmdLineOpen->AddItem(strdup(parm));
				}
			}
		}
	}
	...
	: BApplication(APPLICATION_SIGNATURE)
{
	...
	if (cmdLineOpen)
	{
		for(int i=0;;i++)
		{
			const char *fname = (const char *)cmdLineOpen->ItemAt(i);
			if (!fname) break;
			
			stat("Opening command line document '%s'", fname);
			EditView *ev = CreateEditView(fname);
			if (ev)
				editor.curev = ev;
			else
