...
void EditView::MouseDown(int x, int y, uint32 buttons)
{
EditView *ev = this;

	ev->CancelCommandSeq();
	
	if (buttons & B_TERTIARY_MOUSE_BUTTON)		// right mouse button
	{
...
void EditView::MouseUp(uint32 buttons)		// buttons is NEW buttons down, not which one was released
{
	gesture.Close();
	preventSelExtendUntilUp = false;
}
